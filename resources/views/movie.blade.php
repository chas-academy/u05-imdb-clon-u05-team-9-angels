            ...
            <div class="movie-card-wrapper flex">
                <div>
                    ...
                    @foreach ($comments as $comment)
                        <h2>Rating: {{ $comment->star }}</h2>
                        <h3>Comment: {{ $comment->comment }}</h3>
                    @endforeach
                    <div>
                        @if ($canComment)
                            <form method="POST" action="/movies/comment/create/{{ $movies->id }}">
                                @csrf
                                <input type="text" name="comment" id="comment">
                                <input type="number" name="star" id="star" min="0" max="10">
                                <button
                                    class="mb-5 border border-indigo-500 bg-indigo-500 text-white rounded-md px-4 py-2 mt-2 transition duration-500 ease select-none hover:bg-indigo-600 focus:outline-none focus:shadow-outline"
